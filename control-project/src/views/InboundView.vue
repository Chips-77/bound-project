  ...
  <div class="warehouse-in-page">
    ...
    <el-card class="main-card">
      ...
      <div class="form-header">
        <h2 class="form-title"><i class="iconfont icon-dingdan"></i> 入库单信息</h2>
        <el-divider />
      </div>
      ...
      <el-row :gutter="20" class="form-row">
        ...
        <el-col :span="6">
          <el-form-item label="供应商" prop="supplier">
            <el-select v-model="form.supplier" placeholder="选择供应商">
              <el-option
      ...
      <div class="form-header product-header">
        <h2 class="form-title"><i class="iconfont icon-shangpin"></i> 入库商品列表</h2>
        <el-button
            type="primary"
...
const selectedProducts = ref([]); // 优化：直接存储选中的商品对象
...
const fetchSuppliers = async () => {
  try {
    // 假设API中有获取供应商的方法，如果没有需要创建
    const res = await statisticsApi.getSuppliers();
    suppliers.value = res.data || [];
  } catch (error) {
    console.error('获取供应商列表失败:', error);
    messageManager.error('获取供应商列表失败');
  }
};
...
// 处理返回
const handleBack = () => {
  ElMessageBox.confirm('确定要取消入库单编辑吗？未保存的数据将丢失', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    router.push('/');
    messageManager.info('已取消入库单编辑');
  });
};
...
// 处理选择变化（优化：使用selection-change事件）
...
const handleQuantityChange = (row) => {
  row.amount = row.quantity * row.purchasePrice;
};

// 删除商品
const handleDeleteProduct = (index) => {
  ElMessageBox.confirm('确定要从入库单中移除该商品吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
...
const handleSubmit = async () => {
  ...
  if (form.productList.length === 0) {
    messageManager.warning('请至少添加一个入库商品');
    return;
  }
  if (!form.operator) {
  ...
  try {
    // 调用API提交入库单
    await inboundApi.createInOrder(form);
    loading.close();

    ElMessageBox.success('入库单提交成功！');
